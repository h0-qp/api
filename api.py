...
#print(media("https://www.instagram.com/reel/C88dOFsI01e/?igsh=cnNudDZncWRyazdi","instagram"))
def youtube_downloader(url,site):
	print(url)
	headers = {
		'authority': 'www.y2mate.com',
		'accept': '*/*',
	...
	info = requests.post("https://www.y2mate.com/mates/analyzeV2/ajax",headers=headers,data=data).json()
	title = info["title"]
	vid = info["vid"]
	links = info["links"]["mp4"]
	linkss = [i for i in info["links"]["mp4"].keys()]
	k = links[linkss[0]]["k"]
	data = {
		'vid': vid,
		'k': k,
...
def best_reels(url):
	...
	data = {"url": url}
	return requests.post("https://insta.savetube.me/downloadPostVideo",headers=headers,json=data).json()["post_video_url"]
def instagram_reel(url):
	headers = {
		 "accept": "*/*",
		 ...
		 #"cookie": "gads=ID=c82b30161b4164ea-22e6149cd7e00092:T=1685437011:RT=1685437011:S=ALNI_MYGv9CuhRTcd_ZTV4GEY6VbW5h3sA; gpi=UID=00000c2953f8d723:T=1685437011:RT=1685437011:S=ALNI_MZyOuW8OnkJQNdwcqOjA_XxFVY6ZA; _ga=GA1.2.1251415868.1685437011; _gid=GA1.2.1157888989.1685437011; _gat_gtag_UA_142480840_1=1",
	...
	data = {"url": url,"v":3,"lang":"en"}
	text = requests.post("https://api.downloadgram.org/media",headers=headers,data=data).text
	response = text.replace('\"',"").replace('\h',"h").replace('\ '," ").split('src=')[1].split(' type')[0]
	return response
...
@app.route("/best_reels")
def reelsat2():
	url = request.args.get("url")
	site = "reels"
	return best_reels(url)

@app.route("/instagram_reel")
def reelsat3():
	url = request.args.get("url")
	site = "reels"
	return instagram_reel(url)
...
def youtube():
	url = request.args.get("url")
	site = "youtube_downloader"
	return youtube_downloader(url,"home")
